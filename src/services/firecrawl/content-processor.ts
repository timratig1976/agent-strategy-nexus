...
export class ContentProcessor {
   ...
   */
  extractSummary(data: any[]): string {
   ...
   */
  extractKeywords(data: any[]): string[] {
   ...
   */
  detectTechnologies(data: any[]): string[] {
    if (!data || data.length === 0) return [];
   ...
   */
  private extractCommonWords(text: string, count: number): string[] {
    if (!text) return [];
